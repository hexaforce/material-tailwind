...
export function Configurator() {
  ...
  React.useEffect(() => {
    const stars = fetch('https://api.github.com/repos/creativetimofficial/material-tailwind-dashboard-react')
        ...
        <div className='mb-12'>
          ...
          <div className='mt-3 flex items-center gap-2'>
            {Object.keys(sidenavColors).map((color) => (
              <span key={color} className={`h-6 w-6 cursor-pointer rounded-full border bg-gradient-to-br transition-transform hover:scale-105 ${sidenavColors[color]} ${sidenavColor === color ? 'border-black' : 'border-transparent'}`} onClick={() => setSidenavColor(dispatch, color)} />
            ))}
          </div>
        </div>
        <div className='mb-12'>
